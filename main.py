...
logger = logging.getLogger("tinker_vpn_bot")


async def safe_callback_answer(call: CallbackQuery, text: str = "", show_alert: bool = False):
    try:
...
async def safe_edit_or_answer(call: CallbackQuery, text: str, reply_markup=None):
    try:
...
def kb_buy():
    std_price = plan_meta("standard")[3]
    prm_price = plan_meta("premium")[3]
    fam_price = plan_meta("family")[3]

    return InlineKeyboardMarkup(inline_keyboard=[
        [red_btn(f"📦 Standart • {std_price}₽", "buy:standard")],
        [red_btn(f"⚡ Premium • {prm_price}₽", "buy:premium")],
        [red_btn(f"👨‍👩‍👧‍👦 Family • {fam_price}₽", "buy:family")],
...
async def support_btn(message: Message):
    await message.answer("🆘 Поддержка: https://t.me/zzztruee")


@dp.callback_query(F.data == "menu:main")
async def cb_menu_main(call: CallbackQuery):
    await safe_callback_answer(call)
    await safe_edit_or_answer(call, text_menu(), reply_markup=kb_menu_main())


@dp.callback_query(F.data == "menu:buy")
async def cb_menu_buy(call: CallbackQuery):
    await safe_callback_answer(call)
    await safe_edit_or_answer(call, text_buy_intro(), reply_markup=kb_buy())
...
async def cb_menu_subs(call: CallbackQuery):
    ...
    await safe_edit_or_answer(call, text, reply_markup=kb_sub_with_refresh(call.from_user.id))
...
async def cancel_active_order(call: CallbackQuery):
    ...
    if not order:
        return await safe_callback_answer(call, "Активного заказа нет", show_alert=True)

    db_set_status(order["id"], "cancelled")
    await call.message.answer("❌ Активный заказ отменён. Теперь можно оформить новый заказ.", reply_markup=kb_menu_main())
...
async def noop_call(call: CallbackQuery):
    await safe_callback_answer(call, "Отправь чек следующим сообщением: фото или файлом 👇", show_alert=True)


@dp.message(F.photo | F.document)
async def handle_receipt(message: Message):
    logger.info("receipt received from user_id=%s photo=%s document=%s", message.from_user.id, bool(message.photo), bool(message.document))
    db_upsert_user(message.from_user.id, message.from_user.username, message.from_user.first_name)
