...
async def cancel_active_order(call: CallbackQuery):
    ...
    await call.message.answer("❌ Активный заказ отменён. Теперь можно оформить новый.", reply_markup=kb_menu_main())
...
async def cb_buy_plan(call: CallbackQuery):
    ...
    if active:
        return await call.answer("У тебя уже есть активный заказ. Отправь чек или сначала отмени заказ.", show_alert=True)
    ...
    await call.message.answer(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✅ Я оплатил, сейчас отправлю чек", callback_data=f"noop:{order_id}")],
            [InlineKeyboardButton(text="❌ Отменить заказ", callback_data="menu:cancel_order")],
            [InlineKeyboardButton(text="🏠 В меню", callback_data="menu:main")]
        ])
    )
...
async def noop_call(call: CallbackQuery):
    await call.answer("Теперь отправь сюда чек фото или файлом следующим сообщением 👇", show_alert=True)
...
async def text_fallback(message: Message):
    ...
    if order and not is_admin(message.from_user.id):
        return await message.answer(
            ...
            "После этого заявка уйдёт админу на проверку."
        )
