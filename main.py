...
logger = logging.getLogger("tinker_vpn")


async def safe_answer_callback(call: CallbackQuery, text: str = "", show_alert: bool = False):
    try:
...
async def safe_show_screen(call: CallbackQuery, text: str, reply_markup=None):
    try:
...
def ensure_column(cur, table_name: str, column_name: str, ddl: str):
...
async def cb_menu_main(call: CallbackQuery):
    ...
    await safe_show_screen(call, text_menu(), reply_markup=kb_menu_main())


@dp.callback_query(F.data == "menu:buy")
async def cb_menu_buy(call: CallbackQuery):
    await safe_answer_callback(call)
    await safe_show_screen(call, text_buy_intro(), reply_markup=kb_buy())
...
async def cb_menu_subs(call: CallbackQuery):
    ...
    await safe_show_screen(call, text, reply_markup=kb_sub_with_refresh(call.from_user.id))
...
async def noop_call(call: CallbackQuery):
    await safe_answer_callback(call, "Просто отправь чек следующим сообщением 👇", show_alert=True)
