...
DB_PATH = os.getenv("DB_PATH", "").strip() or os.path.join(DATA_DIR, "orders.sqlite")
BANNER_PATH = os.path.join(BASE_DIR, os.getenv("BANNER_PATH", "banner.jpg"))

STANDARD_KEYS_FILE = os.getenv("STANDARD_KEYS_FILE", "").strip() or os.path.join(DATA_DIR, "standard_keys.txt")
PREMIUM_KEYS_FILE = os.getenv("PREMIUM_KEYS_FILE", "").strip() or os.path.join(DATA_DIR, "premium_keys.txt")
FAMILY_KEYS_FILE = os.getenv("FAMILY_KEYS_FILE", "").strip() or os.path.join(DATA_DIR, "family_keys.txt")
ALLOW_KEY_REUSE = os.getenv("ALLOW_KEY_REUSE", "0").strip().lower() in {"1", "true", "yes", "on"}
...
SUPPORT_URL = "https://t.me/zzztruee"
...
def db_keys_available_count(plan: str) -> int:
    con = db()
    try:
        cur = con.cursor()
    ...
    try:
        ...
        if row:
            ...
            cur.execute(
                ...
                SET used=1,
                    used_at=?,
                    order_id=CASE WHEN ? > 0 THEN ? ELSE order_id END
                WHERE id=?
                """,
                (now_ts(), order_id, order_id, key_id)
            )
            con.commit()
            return key

        if not ALLOW_KEY_REUSE:
            return None
...
def kb_menu_main():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            InlineKeyboardButton(text="⭐ Отзывы", url=REVIEW_LINK)
        ]
    ])
...
async def admin_keys(call: CallbackQuery):
    ...
    text = (
        "🔑 <b>Ключи</b>\n\n"
        f"📦 Standart: <b>{db_keys_available_count('standard')}</b> свободно / <b>{db_keys_count('standard')}</b> всего\n"
        f"⚡ Premium: <b>{db_keys_available_count('premium')}</b> свободно / <b>{db_keys_count('premium')}</b> всего\n"
        f"👨‍👩‍👧‍👦 Family: <b>{db_keys_available_count('family')}</b> свободно / <b>{db_keys_count('family')}</b> всего\n\n"
        "После выдачи ключи не удаляются: они остаются в базе и помечаются как выданные.\n"
