...
TG_CHANNEL = "https://t.me/tinker_vpn"
TG_CHANNEL_USERNAME = "@tinker_vpn"
PRIVATE_GROUP_LINK = "https://t.me/tinker_vpn"
REVIEW_LINK = "https://t.me/tinker_vpn"
...
def payment_text_html() -> str:
    return (
        "💳 <b>Оплата</b>\n\n"
        "✅ <b>Карта:</b>\n"
        "<code>2204320644631782</code>\n\n"
        "📱 <b>Номер:</b>\n"
        "<code>+79224545065</code>\n\n"
        "📎 После оплаты отправь сюда <b>чек / скрин</b>.\n"
        "Я проверю оплату и бот выдаст ключ 🔑"
    )
...
def text_menu() -> str:
    return (
        "🌐 <b>Tinker VPN</b>\n\n"
        "┏ Стабильное быстрое подключение\n"
        "┣ Обход блокировок и цензуры\n"
        "┣ Ручная проверка оплаты админом\n"
        "┗ Подписка выдаётся после подтверждения ✅\n\n"
...
def text_buy_intro() -> str:
    ...
    return (
        ...
        f"• 1 пользователь\n"
        f"• До 3 устройств\n"
        f"• Отдельные хост сервера с топовой скоростью\n"
        f"• <b>{pr_price}₽</b>\n\n"
        f"👨‍👩‍👧‍👦 <b>Family</b>\n"
...
def kb_buy():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📦 Standart", callback_data="buy:standard")],
        [InlineKeyboardButton(text="⚡ Premium", callback_data="buy:premium")],
        [InlineKeyboardButton(text="👨‍👩‍👧‍👦 Family", callback_data="buy:family")],
...
def kb_admin_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="📢 Рассылка", callback_data="admin:broadcast"),
         InlineKeyboardButton(text="🏠 Меню", callback_data="admin:home")],
    ])
...
def kb_admin_users_page(offset: int, total: int, banned: bool = False):
...
def kb_admin_keys():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="➕ Добавить Standart", callback_data="admin:keyadd:standard")],
        [InlineKeyboardButton(text="➕ Добавить Premium", callback_data="admin:keyadd:premium")],
        [InlineKeyboardButton(text="➕ Добавить Family", callback_data="admin:keyadd:family")],
        [InlineKeyboardButton(text="⬅️ Назад", callback_data="admin:home")],
    ])
...
def text_subscription_card(from_user, subs: Optional[list]):
    ...
    if not subs:
        lines.append("У тебя пока нет активных подписок.")
        return "\n".join(lines)
...
async def cmd_admin(message: Message):
    ...
    await message.answer("⚙️ <b>Админ-панель Tinker VPN</b>", reply_markup=kb_admin_menu())
...
async def cb_sub_refresh(call: CallbackQuery):
    ...
    updated = 0
    for sub in subs:
        latest_key = get_latest_key_for_plan(sub["plan"])
        if latest_key:
            updated += db_update_user_plan_key(call.from_user.id, sub["plan"], latest_key)
...
async def admin_home(call: CallbackQuery):
    ...
    await call.answer()
    try:
        await call.message.edit_text("⚙️ <b>Админ-панель Tinker VPN</b>", reply_markup=kb_admin_menu())
    except TelegramBadRequest:
        await call.message.answer("⚙️ <b>Админ-панель Tinker VPN</b>", reply_markup=kb_admin_menu())
...
async def admin_users(call: CallbackQuery):
    ...
    text = "\n".join(lines)
    keyboard = kb_admin_users_page(offset, total, banned=False)
...
async def admin_banned(call: CallbackQuery):
    ...
    try:
        await call.message.edit_text("\n".join(lines), reply_markup=kb_admin_users_page(offset, total, banned=True))
    except TelegramBadRequest:
        await call.message.answer("\n".join(lines), reply_markup=kb_admin_users_page(offset, total, banned=True))
